...
function reserveTable($conn,$order_date,$status,$customer_name,$customer_contact,$customer_email,$numberOfGuests,$reserve_date){
   ...
   // checking to see if mistakes might happen
    ...
    // close stmt function
        ...
        mysqli_stmt_close($stmt);
        echo '<script>window.location="index.php"</script>';  //redirects to home page and displays reservation message
...
function loginSuperAdmin($conn,$username,$pwd){
    ...
    function superAdminPass($conn,$id,$pwd,$newPass){
        $uidExists=idExist($conn,$id);   
        $pwdhashed = $uidExists["password"];
        $checkPwd=password_verify($pwd,$pwdhashed);
        if($checkPwd === false){
            ...
            // executing data from the database
            mysqli_stmt_execute($stmt);
            header("location: ./supaadmin.php?error=SuccessfullyUpdated");
...
<div class="food-menu-box">
    ...
    <div class="food-menu-desc">
        ...
        <p class="food-detail">Quantity: <input type="number" min="1" max="25" name="quantity" class="form-control" value="1" style="width: 60px;"> </p>
        <br>
